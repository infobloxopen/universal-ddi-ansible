...
options:
    ...
    comment:
        description:
            - "Optional. Comment for B(ICMPHealthCheck)."
        type: str
    disabled:
        description:
            - "Optional. Flag which enables/disables B(ICMPHealthCheck). Defaults to I(false)."
        type: bool
    interval:
        description:
    ...
    metadata:
        description:
            - "Output only. B(ICMPHealthCheck) metadata. Defaults to empty object and should be explicitly requested using field selection."
        type: dict
        suboptions:
            used_by:
                description:
    ...
    name:
        description:
            - "Display name of B(ICMPHealthCheck)."
        type: str
        required: true
    retry_down:
        description:
    ...
    tags:
        description:
            - "Optional. The tags for B(ICMPHealthCheck) in JSON format."
        type: dict
    timeout:
        description:
...
EXAMPLES = r"""
    - name: Create an ICMP Health Check
      infoblox.universal_ddi.dtc_health_check_icmp:
        name: "example_icmp_health_check"
        state: present
      register: icmp_health_check

    - name: Create an ICMP Health Check with additional parameters
      infoblox.universal_ddi.dtc_health_check_icmp:
        name: "example_icmp_health_check"
        interval: 15
        retry_down: 2
...
id:
    description:
        - ID of the HealthCheckIcmp object
    type: str
    returned: Always
item:
    description:
        - HealthCheckIcmp object
    type: complex
    returned: Always
    contains:
        comment:
...
class HealthCheckIcmpModule(UniversalDDIAnsibleModule):
    def __init__(self, *args, **kwargs):
        super(HealthCheckIcmpModule, self).__init__(*args, **kwargs)

        exclude = ["state", "csp_url", "api_key", "portal_url", "portal_key", "id"]
        self._payload_params = {k: v for k, v in self.params.items() if v is not None and k not in exclude}
        self._payload = ICMPHealthCheck.from_dict(self._payload_params)
        self._existing = None
    ...
    def find(self):
        if self.params["id"] is not None:
            try:
                resp = HealthCheckIcmpApi(self.client).read(self.params["id"])
                return resp.result
            ...
            resp = HealthCheckIcmpApi(self.client).list(filter=filter)
            ...
            if len(resp.results) > 1:
                self.fail_json(msg=f"Found multiple HealthCheckIcmp: {resp.results}")
    ...
    def create(self):
        ...
        resp = HealthCheckIcmpApi(self.client).create(body=self.payload)
    ...
    def update(self):
        ...
        resp = HealthCheckIcmpApi(self.client).update(id=self.existing.id, body=self.payload)
        return resp.result.model_dump(by_alias=True, exclude_none=True)
    ...
    def delete(self):
        ...
        HealthCheckIcmpApi(self.client).delete(self.existing.id)
    ...
    def run_command(self):
        ...
        try:
            ...
            if self.params["state"] == "present" and self.existing is None:
                item = self.create()
                result["changed"] = True
                result["msg"] = "HealthCheckIcmp created"
            elif self.params["state"] == "present" and self.existing is not None:
                if self.payload_changed():
                    item = self.update()
                    result["changed"] = True
                    result["msg"] = "HealthCheckIcmp updated"
            elif self.params["state"] == "absent" and self.existing is not None:
                self.delete()
                result["changed"] = True
                result["msg"] = "HealthCheckIcmp deleted"
