...
DOCUMENTATION = r"""
---
module: dtc_health_check_pdp_info
short_description: Retrieves DTC PDP Health Checks
description:
    - Retrieve information about existing DTC PDP Health Checks.
version_added: 1.0.0
...
EXAMPLES = r"""
    - name: Get PDP Health Check information by ID
      infoblox.universal_ddi.dtc_health_check_pdp_info:
          id: "{{ health_check_pdp.id }}"

    - name: Get PDP Health Check information by filters (name)
      infoblox.universal_ddi.dtc_health_check_pdp_info:
        filters:
          name: "example_pdp_health_check"

    - name: Get PDP Health Check information by filter query
      infoblox.universal_ddi.dtc_health_check_pdp_info:
        filter_query: "name=='example_pdp_health_check'"

    - name: Get PDP Health Check information by tag filters
      infoblox.universal_ddi.dtc_health_check_pdp_info:
        tag_filters:
          location: "site-1"

    - name: Get PDP Health Check information by tag filter query
      infoblox.universal_ddi.dtc_health_check_pdp_info:
...
try:
    from dtc import HealthCheckPdpApi
    from universal_ddi_client import ApiException, NotFoundException
except ImportError:
    pass  # Handled by UniversalDDIAnsibleModule


class HealthCheckPdpInfoModule(UniversalDDIAnsibleModule):
    def __init__(self, *args, **kwargs):
        super(HealthCheckPdpInfoModule, self).__init__(*args, **kwargs)
        self._existing = None
        self._limit = 1000
    ...
    def find(self):
        ...
        while True:
            try:
                resp = HealthCheckPdpApi(self.client).list(
...
def main():
    ...
    module = HealthCheckPdpInfoModule(
